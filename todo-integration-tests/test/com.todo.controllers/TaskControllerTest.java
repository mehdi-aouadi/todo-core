...
//                bind(UserServiceImpl.class).to(UserService.class);
  ...
  public void postAndGetTaskOkTest() {
    ...
    List<TaskContent> taskContentListAfterPost = jsonSerializer.fromJson(
        response.readEntity(String.class),
        new TypeToken<List<TaskContent>>() {
        }.getType());

    Assert.assertEquals(1, taskContentListAfterPost.size());
    Assert.assertTrue(taskContentListAfterPost.contains(
        TaskMapper.INSTANCE.taskToContent(taskForTest))
    );
  }
}
